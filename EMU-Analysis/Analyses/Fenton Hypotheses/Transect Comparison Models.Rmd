...
par.ar1 <- gamm(par ~  s(day.min, bs = "cc", k = 96) + 
    s(minute, k = 23, fx = T), data = par.data,
    correlation = corAR1(form = ~ 1|t.order))
...
# Fit a model with transect and with penalized splines
par.ar1.transect.penalized <- gamm(par ~  s(day.min, bs = "cc", k = 96) + 
    s(minute) + transect, 
    data = par.data, correlation = corAR1(form = ~ 1|t.order))

# Test whether there are differences in par between transects
AICctab(par.uncorr$lme, par.ar1$lme, par.ar1.transect$lme, 
    par.ar1.transect.penalized$lme, nobs = nrow(par.data))

# Show the summary for the transect order
summary(par.ar1.transect$gam)
...
# plot of autocorrelation in variances
acf(resid(par.ar1.transect$lme, type = "normalized"), lag.max = 96, main = "ACF")
pacf(resid(par.ar1.transect$lme, type = "normalized"), lag.max = 96, main = "pACF")

# Plot the two smoothers
plot(par.ar1.transect$gam, shade = T, residuals = T, ylab = "PFD (µmol m−2 s−1)", 
     main = "PFD")
...
# Plot residuals over time
plot(par.ar1.transect$lme, resid(., type = "normalized") ~ minute)


##### Plot the Model #####

# Add predicted values to the dataset
cbind(par.data, predict(par.ar1.transect$gam, se.fit = T)) %>%
    ...
    ggplot(aes(x = hour)) +
        ...
        # Add observed points
        geom_point(aes(y = par, color = transect), alpha = 0.05) + 
...
transect.coeff <- summary(temp.ar1.transect$gam)$p.table %>% data.frame %>% 
    ...
    # Add the coefficients for the PAR model
    rbind(summary(par.ar1.transect$gam)$p.table %>% data.frame %>% 
        mutate(var = "Par", transect = c("Meadow","Woods"))) %>%
...
write.csv(transect.coeff, "transect.coeff.csv")
