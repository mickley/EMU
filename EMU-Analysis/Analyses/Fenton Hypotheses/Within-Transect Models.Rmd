...
# Make plot of mean and SE of humid (from model), based on transect order

mead.humid.df <- summary(m.humid.ar1.factor$gam)$p.table %>% data.frame
mead.humid.df

mead.humid.df$mean <- mead.humid.df$Estimate
mead.humid.df$mean[2:length(mead.humid.df$Estimate)] <- mead.humid.df$Estimate[1] + mead.humid.df$Estimate[2:length(mead.humid.df$Estimate)]

mead.humid.df$order <-  levels(as.factor(m.humid.data$order))

mead.humid.df <- mead.humid.df %>%
...
wood.humid.df$mean <- wood.humid.df$Estimate
wood.humid.df$mean[2:length(wood.humid.df$Estimate)] <- wood.humid.df$Estimate[1] + wood.humid.df$Estimate[2:length(wood.humid.df$Estimate)]

wood.humid.df$order <-  levels(as.factor(w.humid.data$order))

wood.humid.df <- wood.humid.df %>%
...
comb.humid.df <- rbind(mead.humid.df, wood.humid.df)
...
# Make plot of mean and SE of vwc (from model), based on transect order

mead.vwc.df <- summary(m.vwc.ar1.factor$gam)$p.table %>% data.frame
mead.vwc.df

mead.vwc.df$mean <- mead.vwc.df$Estimate
mead.vwc.df$mean[2:length(mead.vwc.df$Estimate)] <- mead.vwc.df$Estimate[1] + mead.vwc.df$Estimate[2:length(mead.vwc.df$Estimate)]

mead.vwc.df$order <-  levels(as.factor(m.vwc.data$order))

mead.vwc.df <- mead.vwc.df %>%
...
wood.vwc.df$mean <- wood.vwc.df$Estimate
wood.vwc.df$mean[2:length(wood.vwc.df$Estimate)] <- wood.vwc.df$Estimate[1] + wood.vwc.df$Estimate[2:length(wood.vwc.df$Estimate)]

wood.vwc.df$order <-  levels(as.factor(w.vwc.data$order))

wood.vwc.df <- wood.vwc.df %>%
...
comb.vwc.df <- rbind(mead.vwc.df, wood.vwc.df)
...
# Plot the two smoothers
plot(m.par.ar1$gam, shade = T, residuals = T, ylab = "PFD (µmol m−2 s−1)", 
     main = "Meadow PFD")
...
# Make plot of mean and SE of par (from model), based on transect order

mead.par.df <- summary(m.par.ar1.factor$gam)$p.table %>% data.frame
mead.par.df

mead.par.df$mean <- mead.par.df$Estimate
mead.par.df$mean[2:length(mead.par.df$Estimate)] <- mead.par.df$Estimate[1] + mead.par.df$Estimate[2:length(mead.par.df$Estimate)]

mead.par.df$order <-  levels(as.factor(m.par.data$order))

mead.par.df <- mead.par.df %>%
...
wood.par.df <- summary(w.par.ar1.factor$gam)$p.table %>% data.frame
wood.par.df

wood.par.df$mean <- wood.par.df$Estimate
wood.par.df$mean[2:length(wood.par.df$Estimate)] <- wood.par.df$Estimate[1] + wood.par.df$Estimate[2:length(wood.par.df$Estimate)]

wood.par.df$order <-  levels(as.factor(w.par.data$order))

wood.par.df <- wood.par.df %>%
...
comb.par.df <- rbind(mead.par.df, wood.par.df)
...
```{r 'within-transects plots', echo = F, comment = ""}
...
# read in coefficients from between transect model

between <- read.csv("Analyses/Fenton Hypotheses/transect.coeff.csv")

between$upper <- between$Estimate+between$Std..Error
between$lower <- between$Estimate-between$Std..Error
...
comb.temp.plot  + 
    ...
    geom_point(data = filter(between, var=="Temperature"), aes(x = 4.5, y = Estimate, color = transect)) + 
    
    geom_vline(xintercept = 4.25, lty = 2) + 
    
    geom_errorbar(data = filter(between, var=="Temperature"), aes(x = 4.5, ymax = upper, 
                                                                  ymin = lower, color = transect), width = 0.1)
...
comb.humid.plot  + 
    ...
    geom_point(data = filter(between, var=="Humidity"), aes(x = 4.5, y = Estimate, color = transect)) + 
    
    geom_vline(xintercept = 4.25, lty = 2) + 
    
    geom_errorbar(data = filter(between, var=="Humidity"), aes(x = 4.5, ymax = upper, 
                                                                  ymin = lower, color = transect), width = 0.1)
...
comb.vwc.plot  + 
    ...
    geom_point(data = filter(between, var=="VWC"), aes(x = 4.5, y = Estimate, color = transect)) + 
    
    geom_vline(xintercept = 4.25, lty = 2) + 
    
    geom_errorbar(data = filter(between, var=="VWC"), aes(x = 4.5, ymax = upper, 
                                                                  ymin = lower, color = transect), width = 0.1)
...
comb.par.plot  + 
    ...
    geom_point(data = filter(between, var=="Par"), aes(x = 4.5, y = Estimate, color = transect)) + 
    
    geom_vline(xintercept = 4.25, lty = 2) + 
    
    geom_errorbar(data = filter(between, var=="Par"), aes(x = 4.5, ymax = upper, 
                                                                  ymin = lower, color = transect), width = 0.1)


plot_grid(comb.temp.plot + theme(axis.text.x = element_blank(), axis.title.x = element_blank(), legend.position="none"),  
          
          legend,
          
          comb.humid.plot + theme(axis.text.x = element_blank(), axis.title.x = element_blank(), legend.position="none"), 
          
          NULL,
          
          comb.vwc.plot + theme(axis.text.x = element_blank(), axis.title.x = element_blank(), legend.position="none"), 
          
          NULL,
          
          comb.par.plot + theme(legend.position="none"), ncol = 2, rel_widths = c(1, 0.2, 1, 0.2, 1, 0.2))
