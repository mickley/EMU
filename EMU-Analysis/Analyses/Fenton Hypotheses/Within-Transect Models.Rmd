...
# plot of autocorrelation in variances
#acf(resid(m.temp.ar1$lme), lag.max = 96, main = "ACF")
#pacf(resid(m.temp.ar1$lme), lag.max = 96, main = "pACF")

# Reset the layout
#layout(1)

# Plot residuals over time
plot(m.temp.ar1$lme, resid(.) ~ minute)
...
plot(m.temp.ar1$gam, shade = T, residuals = T, main = "Meadow Temp")

# Reset the layout
layout(1)

# Add predicted values to the dataset
cbind(m.temp.data, predict(m.temp.ar1$gam, se.fit = T)) %>%
    ...
    ggplot(aes(x = hour)) +
        ...
        geom_ribbon(aes(ymax = fit + (se.fit * 1.96), 
            ymin = fit - (se.fit * 1.96)), fill = "blue", alpha = 0.7) +
    
        # Add prediction line
        geom_line(aes(y = fit), color = "red") + 
    
        # Add axis labels
        xlab("Hour") + ylab("Temperature (ºC)") + ggtitle("Meadow Temperature") + 
...
# plot of autocorrelation in variances
#acf(resid(w.temp.ar1$lme), lag.max = 96, main = "ACF")
#pacf(resid(w.temp.ar1$lme), lag.max = 96, main = "pACF")

# Reset the layout
#layout(1)

# Plot residuals over time
plot(w.temp.ar1$lme, resid(.) ~ minute)
...
# Plot the two smoothers
plot(w.temp.ar1$gam, shade = T, residuals = T, main = "Woods Temp")

# Reset the layout
layout(1)

# Add predicted values to the dataset
cbind(w.temp.data, predict(w.temp.ar1$gam, se.fit = T)) %>%
    ...
    ggplot(aes(x = hour)) +
        ...
        # Add rectangles for darkness
        geom_rect(data = darkness, aes(xmin = dusk, xmax = dawn, 
            ymin = 3, ymax = 42, x = NULL, y = NULL, color = NULL), 
            fill = "grey85", show.legend = F) + 
        ...
        # Add axis labels
        xlab("Hour") + ylab("Temperature (ºC)") + ggtitle("Woods Temperature") + 
...
# plot of autocorrelation in variances
#acf(resid(m.humid.ar1$lme), lag.max = 96, main = "ACF")
#pacf(resid(m.humid.ar1$lme), lag.max = 96, main = "pACF")

# Reset the layout
#layout(1)

# Plot residuals over time
plot(m.humid.ar1$lme, resid(.) ~ minute)
...
# Plot the two smoothers
plot(m.humid.ar1$gam, shade = T, residuals = T, main = "Woods Humid")

# Reset the layout
layout(1)

# Add predicted values to the dataset
cbind(m.humid.data, predict(m.humid.ar1$gam, se.fit = T)) %>%
    ...
    ggplot(aes(x = hour)) +
        ...
        # Add axis labels
        xlab("Hour") + ylab("Relative Humidity (%)") + ggtitle("Meadow Humidity") +
...
# plot of autocorrelation in variances
#acf(resid(w.humid.ar1$lme), lag.max = 96, main = "ACF")
#pacf(resid(w.humid.ar1$lme), lag.max = 96, main = "pACF")

# Reset the layout
#layout(1)

# Plot residuals over time
plot(w.humid.ar1$lme, resid(.) ~ minute)
...
# Plot the two smoothers
plot(w.humid.ar1$gam, shade = T, residuals = T, main = "Woods Humid")

# Reset the layout
layout(1)

# Add predicted values to the dataset
cbind(w.humid.data, predict(w.humid.ar1$gam, se.fit = T)) %>%
    ...
    ggplot(aes(x = hour)) +
        ...
        # Add rectangles for darkness
        geom_rect(data = darkness, aes(xmin = dusk, xmax = dawn, 
            ymin = 20, ymax = 140, x = NULL, y = NULL, color = NULL), 
            fill = "grey85", show.legend = F) + 
        ...
        # Add axis labels
        xlab("Hour") + ylab("Relative Humidity (%)") + ggtitle("Woods Humidity") + 
...
# plot of autocorrelation in variances
#acf(resid(m.vwc.ar1$lme), lag.max = 96, main = "ACF")
#pacf(resid(m.vwc.ar1$lme), lag.max = 96, main = "pACF")

# Reset the layout
#layout(1)

# Plot residuals over time
plot(m.vwc.ar1$lme, resid(.) ~ minute)
...
# Plot the two smoothers
plot(m.vwc.ar1$gam, shade = T, residuals = T, main = "Meadow VWC")

# Reset the layout
layout(1)

# Add predicted values to the dataset
cbind(m.vwc.data, predict(m.vwc.ar1$gam, se.fit = T)) %>%
    ...
    ggplot(aes(x = hour)) +
        ...
        # Add rectangles for darkness
        geom_rect(data = darkness, aes(xmin = dusk, xmax = dawn, 
            ymin = 0.13, ymax = 0.35, x = NULL, y = NULL, color = NULL), 
            fill = "grey85", show.legend = F) + 
        ...
        # Add axis labels
        xlab("Hour") + ylab("VWC") + ggtitle("Meadow VWC") + 
...
# Show the lme summary for continuous transect order
...
w.vwc.ar1 <- gamm(vwc ~  s(day.min, bs = "cc", k = 96) + 
    s(minute), ctrl = list(niterEM = 0, optimMethod = "L-BFGS-B", 
...
# plot of autocorrelation in variances
#acf(resid(w.vwc.ar1$lme), lag.max = 96, main = "ACF")
#pacf(resid(w.vwc.ar1$lme), lag.max = 96, main = "pACF")

# Reset the layout
#layout(1)

# Plot residuals over time
plot(w.vwc.ar1$lme, resid(.) ~ minute)
...
# Plot the two smoothers
plot(w.vwc.ar1$gam, shade = T, residuals = T, main = "Woods VWC")

# Reset the layout
layout(1)

# Add predicted values to the dataset
cbind(w.vwc.data, predict(w.vwc.ar1$gam, se.fit = T)) %>%
    ...
    ggplot(aes(x = hour)) +
        ...
        # Add rectangles for darkness
        geom_rect(data = darkness, aes(xmin = dusk, xmax = dawn, 
            ymin = 0.05, ymax = 0.3, x = NULL, y = NULL, color = NULL), 
            fill = "grey85", show.legend = F) + 
        ...
        # Add axis labels
        xlab("Hour") + ylab("VWC") + ggtitle("Woods VWC") + 
...
w.vwc.ar1.factor <- gamm(vwc ~  s(day.min, bs = "cc", k = 96) + 
    s(minute) + factor(order), ctrl = list(niterEM = 0, optimMethod = "L-BFGS-B", 
...
#    s(minute) + order, ctrl = list(niterEM = 0, optimMethod = "L-BFGS-B", 
...
# plot of autocorrelation in variances
#acf(resid(m.par.ar1$lme), lag.max = 96, main = "ACF")
#pacf(resid(m.par.ar1$lme), lag.max = 96, main = "pACF")
...
# Plot the two smoothers
plot(m.par.ar1$gam, shade = T, residuals = T, main = "Meadow PAR")

# Reset the layout
layout(1)

# Add predicted values to the dataset
cbind(m.par.data, predict(m.par.ar1$gam, se.fit = T)) %>%
    ...
    ggplot(aes(x = hour)) +
        ...
        # Add axis labels
        xlab("Hour") + ylab("PAR") + ggtitle("Meadow PAR") + 
...
# Plot residuals over time
plot(w.par.ar1$lme, resid(.) ~ minute)
...
# Plot the two smoothers
plot(w.par.ar1$gam, shade = T, residuals = T, main = "Woods PAR")

# Reset the layout
layout(1)

# Add predicted values to the dataset
cbind(w.par.data, predict(w.par.ar1$gam, se.fit = T)) %>%
    ...
    ggplot(aes(x = hour)) +
        ...
        # Add rectangles for darkness
        geom_rect(data = darkness, aes(xmin = dusk, xmax = dawn, 
            ymin = -10, ymax = 1700, x = NULL, y = NULL, color = NULL), 
            fill = "grey85", show.legend = F) + 
        ...
        # Add axis labels
        xlab("Hour") + ylab("PAR") + ggtitle("Woods PAR") + 
