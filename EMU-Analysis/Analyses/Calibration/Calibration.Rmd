...
light.hobo.comp %>% ggplot(aes(x = umol, y = umol.quad)) +
        ...
        # Add axes
        xlab(bquote(bold('Hobo PFD (µmol '~m^-2 ~s^-1~')'))) +
        ylab(bquote(bold('EMU PFD (µmol '~m^-2 ~s^-1~')'))) +
...
# Rename the intercept coefficient
names(gold.quad.2$coefficients) <- c("intercept", "soil", "soil.squared")

# Write the calibration coefficients to a CSV
write.csv(data.frame(as.list(gold.quad.2$coefficients)),
    "Analyses/Calibration/soil-calibration.csv", row.names = F)
