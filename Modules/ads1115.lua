...
local CONFIG_COMP_QUEUE_1       = 0x0000 -- 1 conversion exceeding threshholds before ALERT pin is activated
local CONFIG_COMP_QUEUE_2       = 0x0001 -- 2 conversions exceeding threshholds before ALERT pin is activated
local CONFIG_COMP_QUEUE_4       = 0x0002 -- 4 conversion exceeding threshholds before ALERT pin is activated
local CONFIG_COMP_DISABLE       = 0x0003 -- Comparator disabled, and ALERT pin pulled high (default)

-- Configure comparator Latching (Bit 2)
local CONFIG_COMP_NOLATCH       = 0x0000 -- ALERT pin will deactivate according to comparator mode (default)
local CONFIG_COMP_LATCH         = 0x0004 -- ALERT pin latches on activation and must be manually cleared
 
-- Configure the alert pin (Bit 3)
-- Sets whether the ALERT pin is set to be low or high by the comparator
local CONFIG_COMP_ALERT_LOW     = 0x0000 -- Active low.  When ALERT is activated it will be low (default)
local CONFIG_COMP_ALERT_HIGH    = 0x0008 -- Active high.  When ALERT is activated it will be high
...
local CONFIG_COMP_MODE_HYSTER   = 0x0000 -- Traditional comparator mode with hysteresis (default)
local CONFIG_COMP_MODE_WINDOW   = 0x0010 -- Window comparator mode


-- Configuration of the sampling rate (Bits 5-7)
-- This sets how fast sampling occurs.  
local CONFIG_RATE_8             = 0x0000 -- 8 samples per second
local CONFIG_RATE_16            = 0x0020 -- 16 samples per second
local CONFIG_RATE_32            = 0x0040 -- 32 samples per second
local CONFIG_RATE_64            = 0x0060 -- 64 samples per second
local CONFIG_RATE_128           = 0x0080 -- 128 samples per second (default)
local CONFIG_RATE_250           = 0x00A0 -- 250 samples per second
local CONFIG_RATE_475           = 0x00C0 -- 475 samples per second
local CONFIG_RATE_860           = 0x00E0 -- 860 samples per second

-- Configuration of the (Bit 8)
local CONFIG_MODE_CONTINUOUS    = 0x0000 -- Convert values continuously
local CONFIG_MODE_SINGLE        = 0x0100 -- Single shot mode, power down afterwards (default)


-- Configuration of the PGA or voltage range/sensitivity (Bits 9-11)
local CONFIG_PGA_6144           = 0x0000 -- Gain 2/3: +/- 6.144 volts: 0.1875 mV resolution
local CONFIG_PGA_4096           = 0x0200 -- Gain 1:   +/- 4.096 volts: 0.125 mV resolution
local CONFIG_PGA_2048           = 0x0400 -- Gain 2:   +/- 2.048 volts: 0.0625 mV resolution (default)
local CONFIG_PGA_1024           = 0x0600 -- Gain 4:   +/- 1.024 volts: 0.03125 mV resolution
local CONFIG_PGA_512            = 0x0800 -- Gain 8:   +/- 0.512 volts: 0.015625 mV resolution
local CONFIG_PGA_256            = 0x0A00 -- Gain 16:  +/- 0.256 volts: 0.007813 mV resolution

-- Configuration (Bits 12-14)
local CONFIG_MUX_DIFF_01        = 0x0000 -- Difference between A0 (+) and A1 (-) (default)
local CONFIG_MUX_DIFF_03        = 0x1000 -- Difference between A0 (+) and A3 (-)
local CONFIG_MUX_DIFF_13        = 0x2000 -- Difference between A1 (+) and A3 (-)
local CONFIG_MUX_DIFF_23        = 0x3000 -- Difference between A2 (+) and A3 (-)
local CONFIG_MUX_SINGLE_0       = 0x4000 -- Difference between A0 (+) and Ground
local CONFIG_MUX_SINGLE_1       = 0x5000 -- Difference between A1 (+) and Ground
local CONFIG_MUX_SINGLE_2       = 0x6000 -- Difference between A2 (+) and Ground
local CONFIG_MUX_SINGLE_3       = 0x7000 -- Difference between A3 (+) and Ground

-- Bit 15
local CONFIG_OS_NULL            = 0x0000 -- Unused, no effect
local CONFIG_OS_CONVERT         = 0x8000 -- Begin a single conversion when in power down mode (default)
...
function M.init(sda, scl)
    ...
    -- Set up the defaults
    comp = CONFIG_COMP_DISABLE -- Disable the comparator
    rate = CONFIG_RATE_128 -- 128 samples per second
    pga = CONFIG_PGA_6144 -- +/- 6.144 volts of range
...
function M.setRate(setrate)
    ...
    elseif setrate == 64 then
        ...
        -- 
        rate = CONFIG_RATE_128
        
    elseif setrate == 128 then

        -- 
        rate = CONFIG_RATE_250
        
    elseif setrate == 250 then

        -- 
        rate = CONFIG_RATE_475
...
function M.readADC(channel)
    ...
    elseif channel == 10 then
        mux = CONFIG_MUX_DIFF_01
    elseif channel == 30 then
        mux = CONFIG_MUX_DIFF_03
    elseif channel == 31 then
        mux = CONFIG_MUX_DIFF_13
    elseif channel == 32 then
        mux = CONFIG_MUX_DIFF_23
    else
    ...
    local config = 0
    config = bit.bor(comp, rate, CONFIG_MODE_SINGLE, pga, mux, CONFIG_OS_CONVERT)
